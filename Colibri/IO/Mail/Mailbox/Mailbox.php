    ...
    namespace Colibri\IO\Mail\Mailbox {
        ...
        class Mailbox {
            ...
            public function switchMailbox($imapPath = '') {
                ...
                $_imapStream = @imap_reopen($this->getImapStream(), $imapPath);
                if(!$_imapStream) {
            ...
            protected function initMailPart(Mail $mail, $partStructure, $partNum, $markAsSeen = true) {
                ...
                // attachments
                if($partStructure->ifid){
                    $attachmentId = trim($partStructure->id, " <>");
                }
                else {
                    $attachmentId = isset($params['filename']) || isset($params['name']) ? mt_rand() . mt_rand() : null;
                }
